...
Ngoài video, form upload ảnh nhận riêng hai nhóm LOW/reference, sắp xếp mỗi nhóm theo thời gian chụp trong metadata rồi ghép tuần tự trước khi review; sau đó có thể chọn dùng chung `job_id` hoặc mỗi cặp một `job_id` riêng.
Ảnh HEIC/HEIF được đọc metadata trước, tự động chuyển thành PNG rồi mới đưa vào luồng review/upload Drive.
Dashboard thống kê đóng góp theo người chụp bằng cách lấy các cặp đang tồn tại trên Google Drive rồi đối chiếu row metadata mới nhất trong database; row lịch sử của file đã xóa không được tính.
